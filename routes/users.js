 ...
 * @param {number} next - Goes on to the next stage
 * @returns {object} - database information
 */
async function emailSearch(ctx, next) {
  // adds pagination, partial response, etc.
...
async function login(ctx) {
  // returns any details needed by the client
...
async function getAll(ctx) {
  ...
  if (!permission.granted) {
    ...
    // ensures params are integers
    limit = parseInt(limit);
    page = parseInt(page);
    
    // validates pagination values to ensure they are sensible
    ...
    if (result.length) {
      if (fields !== null) {
        // ensures the fields are contained in an array
        // a single field gets passed as a string
        if (!Array.isArray(fields)) {
          fields = [fields];
        }
        // it then then filters each row in the array of results
        // but by only including the specified fields
